...
export const useStoreValue = <DataModel extends object, K extends keyof DataModel>(path: K): [DataModel[K], StoreValueSetter<DataModel[K]>] => {
	...
	const [state, setState] = useState<{ store: Store<DataModel>; path: K; value: DataModel[K] }>(() => ({
		store,
		path,
	...
	let valueToReturn: DataModel[K] = state.value;
