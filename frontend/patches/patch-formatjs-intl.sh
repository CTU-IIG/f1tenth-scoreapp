...
# This is temporary patch script working only for @formatjs/intl version 1.14.1
# This script must be run from the frontend root

if ! grep '"version": "1.14.1"' "node_modules/@formatjs/intl/package.json" 1>/dev/null; then
	echo "@formatjs/intl version 1.14.1 not found (node_modules/@formatjs/intl/package.json)"
	exit 1
fi
...
if grep "fractionalSecondDigits" "node_modules/@formatjs/intl/lib/src/dateTime.js" 1>/dev/null && grep "fractionalSecondDigits" "node_modules/@formatjs/intl/src/dateTime.js" 1>/dev/null; then
	echo "skipping @formatjs/intl version 1.14.1 patching (already patched)"
	exit 0
fi
